...
const IndividualSubject: React.FC<SubjectSummaryProps> = ({}) => {
  ...
  // --- ADD: mapping for EOL-only param (display/right => eol/left) ---
// map: normalized display/right-side -> left-side EOL value
// Source: array of pairs (display, eolLeft)
// --- BE SURE to place these near your other helpers (e.g., below toNumber) ---

const EOL_PAIRS: Array<[string, string]> = [
  ["Physics", "physics"],
  ["Unit of enquiry", "Uoi"],
  ["Biology", "Biology"],
  ["Geography", "Geography"],
  ["History", "History"],
  ["Chemistry", "Chemistry"],
  ["Environmental systems and societies", "E.s.s."],
  ["Economics", "Economics"],
  ...
  ["English as second language or english", "Esl"],
  ["Career program", "Cp- pps"],
  ["German ab initio", "German ab initio"],
  ["Geogaphy", "Geogaphy"],                // preserve variant if backend expects it
  ["Digital design", "Digital design"],
  ["French", "French"],
  ["Spanish", "Spanish"],
  ["French SL/ French Ab / French Ab initio", "French sl & french ab"],
  ["Music Dance Drama", "Mdd"],
  ["English A: language & literature", "English language & literature"],
  ["Environmental systems and societies", "Ess"],
  ["Visual art", "Visual art"],
  ["Design tech", "Design tech"],
  ["Digitalsociety", "Digitalsociety"],
  ["Hindi", "Hindi"],
  ["French Ab/ French B SL", "French ab & french b sl"],
  ["Spanish ab & spanish b sl", "Spanish ab & spanish b sl"],
  ["German ab initio", "German ab"],
  ["German bsl", "German bsl"],
  ["Englishb", "Englishb"],
  ["Hindi b sl", "Hindi b sl"],
  ["Theory of knowledge", "Tok"]
];

const buildEolMap = (pairs: Array<[string, string]>) => {
  const map: Record<string, string> = {};
  const seen = new Map<string, string>();
  for (const [display, left] of pairs) {
    const key = String(display ?? "").trim().toLowerCase();
    if (!key) continue;
    if (seen.has(key)) {
      // keep first entry; log duplicate so you can fix source if needed
      console.warn(`Duplicate mapping key detected for "${display}" (normalized "${key}"). Keeping first entry: "${seen.get(key)}"`);
      continue;
    }
    seen.set(key, left);
    map[key] = left;
...
// THE MAP you should use everywhere
const EOL_SUBJECT_MAP_SAFE = buildEolMap(EOL_PAIRS);

// Normalizer + lookup
const normalizeKey = (s?: string) => String(s ?? "").trim().toLowerCase();

function getEolSubjectParam(displaySubject?: string) {
  const key = normalizeKey(displaySubject);
  if (!key) return displaySubject ?? "";
  // direct lookup on the safe map
  if (EOL_SUBJECT_MAP_SAFE.hasOwnProperty(key)) return EOL_SUBJECT_MAP_SAFE[key];
  // fallback: remove dots/extra spaces and try again
  const simple = key.replace(/[.\s]+/g, " ").trim();
  if (EOL_SUBJECT_MAP_SAFE.hasOwnProperty(simple)) return EOL_SUBJECT_MAP_SAFE[simple];
  // final fallback: return original (so other endpoints keep using selectedSubject)
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
         ...
         const eolSubjectParam = getEolSubjectParam(selectedSubject);
        const urls = [
          `${base}/assessments/eol/${enrollmentid}~${encodeURIComponent(eolSubjectParam)}/`,
